...
then collect this parameters into a struct, now that a ptr of this struct and pass it to pthread_create()
...
void *func0(void* arg) //Here only one argument is allowed
{
	   char *y=(char*)arg;
	   printf("%c\n",*y);
}
...
void *func2(void* arg) //Here only one argument is allowed
{
	   char *y=(char*)arg;
	   printf("%s\n",y);
}

void *func3(void* arg) //Here only one argument is allowed
{
	   int y=*((int*)arg);
	   printf("%d\n",y);
}
